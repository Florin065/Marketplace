<template>
          ...
          <div class="row">
              <q-card-section
                  ...
                  <q-card style="border-radius: 8px; border: 1px dashed #E0E0E0; display: flex; align-items: center; justify-content: center; width: 160px; height: 160px;">
                      ...
                      >
                          <div v-if="featuredImage === null" style="margin-right: 35px; margin-top: 35px">
                          ...
                          <div>
                              <q-card>
                                  <q-file
                                      ...
                                      >
                                      <div v-if="variant.image === null" style="margin-right: 40px; margin-top: 45px">
                                          <q-icon name="add_photo_alternate" size="100px" color="grey-6" />
                                      </div>
                                      <div v-else>
  ...
  // Watch for changes in the product SKU and update variants' SKUs
  watch(() => productData.value.SKU, (newSKU) => {
    productData.value.variants.forEach((variant, index) => {
      variant.sku = `${newSKU}-${index + 1}`;
    });
  });
  ...
  const addVariant = () => {
    const newVariantIndex = productData.value.variants.length;
    const newSKU = productData.value.SKU !== '' ? `${productData.value.SKU}-${newVariantIndex + 1}` : '';
  ...
  // Update SKUs for all variants
  const updateVariantSKUs = () => {
    if (productData.value.SKU !== '') {
      productData.value.variants.forEach((variant, index) => {
        variant.sku = `${productData.value.SKU}-${index + 1}`;
      });
    } else {
      productData.value.variants.forEach((variant, index) => {
        variant.sku = '';
